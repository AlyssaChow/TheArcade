...
class SnakeEngine extends SurfaceView implements Runnable {
    ...
    // For tracking movement Heading
    public enum Heading2 {UP, RIGHT, DOWN, LEFT}

    // Start by heading to the right
    private Heading heading2 = Heading.RIGHT;
...
// We will draw the frame much more often
    ...
    private int[] snakeXs;
    private int[] snakeYs;
    private int[] snakeXs2;
    private int[] snakeYs2;
...
// Is the game currently playing?
    ...
    public SnakeEngine(Context context, Point size) {
        ...
        snakeXs = new int[200];
        snakeYs = new int[200];
        snakeXs2 = new int[200];
        snakeYs2 = new int[200];
    ...
    public void newGame() {
        ...
        snakeXs2[0] = NUM_BLOCKS_WIDE / 2;
        snakeYs2[0] = numBlocksHigh / 2;
    ...
    private void moveSnake2() {
        ...
        for (int i = snakeLength2; i > 0; i--) {
            // Start at the back and move it
            // to the position of the segment in front of it
            snakeXs2[i] = snakeXs2[i - 1];
            snakeYs2[i] = snakeYs2[i - 1];
        ...
        double r = Math.sqrt(Math.pow(bobX - snakeXs2[0], 2) + Math.pow(bobY - snakeYs2[0], 2));

        double distanceToPlayerX1 = bobX - snakeXs2[0];
        double distanceToPlayerY1 = bobY - snakeYs2[0];

        // Calculate (absolute) distance between enemy (this) and player
        double distanceToPlayer = r;
        ...
        if (distanceToPlayer > 0) { // Avoid division by zero
            snakeXs2[0] += distanceToPlayerX1;
            snakeYs2[0] += distanceToPlayerY1;

        }
        if (snakeXs2[0] == bobX && snakeYs2[0] == bobY) {
            eatBob2();
        }
    ...
    public void draw() {
        ...
        if (surfaceHolder.getSurface().isValid()) {
            ...
            canvas.drawText("Score2:" + score2, 10, 150, paint);
            ...
            for (int j = 0; j < snakeLength2; j++) {
                canvas.drawRect(snakeXs2[j] * blockSize,
                        (snakeYs2[j] * blockSize),
                        (snakeXs2[j] * blockSize) + blockSize,
                        (snakeYs2[j] * blockSize) + blockSize,
                        paint2);
            }

            // Set the color of the paint to draw Bob red
            paint.setColor(Color.argb(255, 255, 25, 200));
